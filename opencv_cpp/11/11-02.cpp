...
int main()
{
  ...
  for (int i = 0; i < num_data; i++) {
    ...
    double r = 0.6*rng.gaussian(1.0) + 5.0; //正規分布（半径）
    double angle = 2.0*M_PI*rng.uniform(0.0, 1.0); //一様分布（角度）

    class2.at<double>(i, 0) = r*cos(angle);
    class2.at<double>(i, 1) = r*sin(angle);
  }
  for (int i = 0; i < 2 * num_data; i++) {
    if (i < num_data)
      labels.at<int>(i) = 0;//前半クラス1のラベル値
    else
      labels.at<int>(i) = 1;//後半クラス2のラベル値
  }
