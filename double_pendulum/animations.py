...
def create_comment(example):
    ...
    for i, b in enumerate((example._b1, example._b2), 1):
        s = 'Pendulum {i}: position: ({b.x:.2f}, {b.y:.2f}), mass: {b.m:.2f}'.format(
            i=i, b=b)
        status.append(s)
    return '\n'.join(status)
...
def multi_animation(systems, ex, fig_size=(8, 8), hide_axes=True, filename=None):
    ...
    for k in range(len(systems)):
        ...
        m1s.append(ax.plot([], [], 'o', ms=4 *
                           ex.M[0, 0], c='C{}'.format(k))[0])
        m2s.append(ax.plot([], [], 'o', ms=4 *
                           ex.M[1, 1], c='C{}'.format(k))[0])
...
def __run_basic_example():
    ...
    while True:
        ...
        try:
            r = simulation.simulate(
                rs, method=simulation.Euler, step_size=0.005)
            break
    ...
    mms = simulation.simulate_multiple_methods(
        rs, mtd, duration=20, step_size=0.01)
