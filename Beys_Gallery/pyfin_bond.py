...
def Bond_Convexity(Yield, Maturity, CouponRate, FaceValue):
    ...
    Dispersion = pol.polyval(1.0 / (1.0 + 0.01 * Yield), np.hstack((0.0, Coef)))                  / Price
...
#   割引債利回りの計算
P = Bond[:,0]
C = F * np.identity(Bond.shape[0])     + np.tril(np.transpose(np.tile(0.01 * Bond[:, 2] * F, (Bond.shape[0], 1))))
V = lin.solve(C, P)
