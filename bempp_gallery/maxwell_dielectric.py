...
wavenumbers = [k0 * np.sqrt(er * mr)
               for er, mr in zip(rel_permittivities, rel_permeabilities)]
...
def tangential_trace(point, n, domain_index, result):
    value = np.sqrt(vacuum_permittivity) * polarization * \
...
def neumann_trace(point, n, domain_index, result):
    value = np.cross(direction, polarization) * 1j * k0 * \
        np.sqrt(vacuum_permittivity) * \
        np.exp(1j * k0 * np.dot(point, direction))
...
transfer_operators = np.empty(
    (number_of_scatterers, number_of_scatterers), dtype=np.object)
...
for i in range(number_of_scatterers):
    filter_operators[i] = .5 * identity_operators[i] - \
        scaled_interior_operators[i]
...
for i in range(number_of_scatterers):
    incident_trace_data[i] = (
        bempp.api.GridFunction(
            blocked_operator.domain_spaces[2 * i], fun=tangential_trace, dual_space=blocked_operator.dual_to_range_spaces[2 * i]),
...
bempp.api.enable_console_logging()
sol, info, _ = bempp.api.linalg.gmres(
...
for i in range(number_of_scatterers):
    ...
    interior_trace_data[i][1] += np.sqrt(rel_permeabilities[i]
                                         ) * sol[2 * i + 1]
...
for i in range(number_of_scatterers):
    ...
    mpot_int = bempp.api.operators.potential.maxwell.magnetic_field(interior_trace_data[i][0].space, int_points,
                                                                    wavenumbers[i])
    epot_ext = bempp.api.operators.potential.maxwell.electric_field(
        sol[2 * i + 1].space, ext_points, k0)
    mpot_ext = bempp.api.operators.potential.maxwell.magnetic_field(
        sol[2 * i].space, ext_points, k0)

    exterior_values += -epot_ext * sol[2 * i + 1] - mpot_ext * sol[2 * i]
    interior_values[i] = (epot_int * interior_trace_data[i]
                          [1] + mpot_int * interior_trace_data[i][0])
...
# Increase the figure size in the notebook
plt.rcParams['figure.figsize'] = (20, 16)

# First compute the scattered field
scattered_field = np.empty((3, points.shape[1]), dtype='complex128')
scattered_field[:, :] = np.nan
scattered_field[:, ext_indices] = 1. / \
    np.sqrt(vacuum_permittivity) * exterior_values

# Now compute the total field
total_field = np.empty((3, points.shape[1]), dtype='complex128')

for i in range(exterior_values.shape[1]):
    total_field[:, ext_indices[i]] = scattered_field[:,
                                                     ext_indices[i]] + plane_wave(points[:, ext_indices[i]])

for i in range(number_of_scatterers):
    # Add interior contributions
    total_field[:, index_sets[i]] = 1. / \
        np.sqrt(rel_permittivities[i] *
                vacuum_permittivity) * interior_values[i]
...
plt.savefig("./maxwell_dielectric.png")
plt.show()
