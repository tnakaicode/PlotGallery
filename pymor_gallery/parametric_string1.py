...
for mu, rom_sobtp in zip(mu_list, roms_sobtp):
    ...
    print(f'mu = mu')
    print(
        f'    SOBTp relative H_2-error:    {err_sobtp.h2_norm(mu=mu) / so_sys.h2_norm(mu=mu):e}')
    if config.HAVE_SLYCOT:
        print(
            f'    SOBTp relative H_inf-error:  {err_sobtp.hinf_norm(mu=mu) / so_sys.hinf_norm(mu=mu):e}')
    print(
        f'    SOBTp relative Hankel-error: {err_sobtp.hankel_norm(mu=mu) / so_sys.hankel_norm(mu=mu):e}')
...
for mu, rom_sobtv in zip(mu_list, roms_sobtv):
    ...
    print(f'mu = mu')
    print(
        f'    SOBTv relative H_2-error:    {err_sobtv.h2_norm(mu=mu) / so_sys.h2_norm(mu=mu):e}')
    if config.HAVE_SLYCOT:
        print(
            f'    SOBTv relative H_inf-error:  {err_sobtv.hinf_norm(mu=mu) / so_sys.hinf_norm(mu=mu):e}')
    print(
        f'    SOBTv relative Hankel-error: {err_sobtv.hankel_norm(mu=mu) / so_sys.hankel_norm(mu=mu):e}')
...
for mu, rom_sobtpv in zip(mu_list, roms_sobtpv):
    ...
    print(f'mu = mu')
    print(
        f'    SOBTpv relative H_2-error:    {err_sobtpv.h2_norm(mu=mu) / so_sys.h2_norm(mu=mu):e}')
    if config.HAVE_SLYCOT:
        print(
            f'    SOBTpv relative H_inf-error:  {err_sobtpv.hinf_norm(mu=mu) / so_sys.hinf_norm(mu=mu):e}')
    print(
        f'    SOBTpv relative Hankel-error: {err_sobtpv.hankel_norm(mu=mu) / so_sys.hankel_norm(mu=mu):e}')
...
for mu, rom_sobtvp in zip(mu_list, roms_sobtvp):
    ...
    print(f'mu = mu')
    print(
        f'    SOBTvp relative H_2-error:    {err_sobtvp.h2_norm(mu=mu) / so_sys.h2_norm(mu=mu):e}')
    if config.HAVE_SLYCOT:
        print(
            f'    SOBTvp relative H_inf-error:  {err_sobtvp.hinf_norm(mu=mu) / so_sys.hinf_norm(mu=mu):e}')
    print(
        f'    SOBTvp relative Hankel-error: {err_sobtvp.hankel_norm(mu=mu) / so_sys.hankel_norm(mu=mu):e}')
...
for mu, rom_sobtfv in zip(mu_list, roms_sobtfv):
    ...
    print(f'mu = mu')
    print(
        f'    SOBTfv relative H_2-error:    {err_sobtfv.h2_norm(mu=mu) / so_sys.h2_norm(mu=mu):e}')
    if config.HAVE_SLYCOT:
        print(
            f'    SOBTfv relative H_inf-error:  {err_sobtfv.hinf_norm(mu=mu) / so_sys.hinf_norm(mu=mu):e}')
    print(
        f'    SOBTfv relative Hankel-error: {err_sobtfv.hankel_norm(mu=mu) / so_sys.hankel_norm(mu=mu):e}')
...
for mu, rom_sobt in zip(mu_list, roms_sobt):
    ...
    print(f'mu = mu')
    print(
        f'    SOBT relative H_2-error:    {err_sobt.h2_norm(mu=mu) / so_sys.h2_norm(mu=mu):e}')
    if config.HAVE_SLYCOT:
        print(
            f'    SOBT relative H_inf-error:  {err_sobt.hinf_norm(mu=mu) / so_sys.hinf_norm(mu=mu):e}')
    print(
        f'    SOBT relative Hankel-error: {err_sobt.hankel_norm(mu=mu) / so_sys.hankel_norm(mu=mu):e}')
...
for mu, rom_bt in zip(mu_list, roms_bt):
    ...
    print(f'mu = mu')
    print(
        f'    BT relative H_2-error:    {err_bt.h2_norm(mu=mu) / so_sys.h2_norm(mu=mu):e}')
    if config.HAVE_SLYCOT:
        print(
            f'    BT relative H_inf-error:  {err_bt.hinf_norm(mu=mu) / so_sys.hinf_norm(mu=mu):e}')
    print(
        f'    BT relative Hankel-error: {err_bt.hankel_norm(mu=mu) / so_sys.hankel_norm(mu=mu):e}')
...
for mu, rom_irka in zip(mu_list, roms_irka):
    ...
    print(f'mu = mu')
    print(
        f'    IRKA relative H_2-error:    {err_irka.h2_norm(mu=mu) / so_sys.h2_norm(mu=mu):e}')
    if config.HAVE_SLYCOT:
        print(
            f'    IRKA relative H_inf-error:  {err_irka.hinf_norm(mu=mu) / so_sys.hinf_norm(mu=mu):e}')
    print(
        f'    IRKA relative Hankel-error: {err_irka.hankel_norm(mu=mu) / so_sys.hankel_norm(mu=mu):e}')
...
for mu, rom_sor_irka in zip(mu_list, roms_sor_irka):
    ...
    print(f'mu = mu')
    print(
        f'    SORIRKA relative H_2-error:    {err_sor_irka.h2_norm(mu=mu) / so_sys.h2_norm(mu=mu):e}')
    if config.HAVE_SLYCOT:
        print(
            f'    SORIRKA relative H_inf-error:  {err_sor_irka.hinf_norm(mu=mu) / so_sys.hinf_norm(mu=mu):e}')
    print(
        f'    SORIRKA relative Hankel-error: {err_sor_irka.hankel_norm(mu=mu) / so_sys.hankel_norm(mu=mu):e}')
