...
def discretize_pymor():
    ...
    problem = InstationaryProblem(
        ...
        StationaryProblem(
            ...
            rhs=ConstantFunction(value=100., dim_domain=2) *
            ExpressionParameterFunctional('sin(10*pi*t[0])', {'t': 1}),
    ...
    fom, _ = discretize_instationary_cg(
        analytical_problem=problem, diameter=1. / GRID_INTERVALS, nt=NT)
...
def _discretize_fenics():
    ...
    mat0 = h1_mat.copy()
    mat0.zero()
    bottom_mat = df.assemble(
        bottom_diffusion * df.inner(df.nabla_grad(u), df.nabla_grad(v)) * df.dx)
    top_mat = df.assemble(
        top_diffusion * df.inner(df.nabla_grad(u), df.nabla_grad(v)) * df.dx)
...
def reduce_pod(fom, reductor, parameter_space, snapshots, basis_size):
    ...
    basis, singular_values = pod(
        snapshots, modes=basis_size, product=fom.h1_0_semi_product)
...
def main(BACKEND, ALG, SNAPSHOTS, RBSIZE, TEST):
    ...
    reductor = ParabolicRBReductor(
        fom, product=fom.h1_0_semi_product, coercivity_estimator=coercivity_estimator)
    ...
    if ALG == 'greedy':
        ...
        rom = reduce_adaptive_greedy(
            fom, reductor, parameter_space, SNAPSHOTS, RBSIZE)
    elif ALG == 'pod':
    ...
    results = reduction_error_analysis(
        ...
        error_norms=[lambda U: DT *
                     np.sqrt(np.sum(fom.h1_0_semi_norm(U)[1:]**2))],
...
if __name__ == '__main__':
    ...
    BACKEND, ALG, SNAPSHOTS, RBSIZE, TEST = BACKEND.lower(
    ), ALG.lower(), int(SNAPSHOTS), int(RBSIZE), int(TEST)
