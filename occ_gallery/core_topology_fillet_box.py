...
def split_box(box, pln):
    splitter = BOPAlgo_Splitter()
    splitter.AddArgument(box)
    splitter.AddTool(pln)
    splitter.Perform()
...
pln = make_plane(gp_Pnt(0, 0, 10), gp_Vec(
    0, 0.5, 0.8), -1000, 1000, -1000, 1000)
...
for shp in [box1_faces[3], box1_faces[2]]:
    bild.Add(box1_shell, shp)

# Find Edge that the two faces share
find_edge = LocOpe_FindEdges(box1_faces[3], box1_faces[2])
