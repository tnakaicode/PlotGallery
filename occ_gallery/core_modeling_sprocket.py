...
def build_tooth():
    ...
    x_distance = cos(roller_contact_angle / 2.0) * \
        (profile_radius + tooth_radius)
    y_distance = sin(roller_contact_angle / 2.0) * \
        (profile_radius + tooth_radius)
...
def clone_tooth(base_shape):
    ...
    aggregated_shape = BRepAlgoAPI_Fuse(
        aggregated_shape, cylinder.Shape()).Shape()
...
def mounting_holes(base):
    ...
    for i in range(0, mounting_hole_count):
        ...
        cylinder = BRepPrimAPI_MakeCylinder(
            center_axis, hole_radius, thickness).Shape()
