...
def make_base_spline():
    ...
    pnts.SetValue(2, gp_Pnt(-7, 2, 0))
    pnts.SetValue(3, gp_Pnt(-6, 3, 0))
    pnts.SetValue(4, gp_Pnt(-4, 3, 0))
...
def main():
    ...
    for d, ang in zip(offsets, angles_deg):
        if d == 0.0:
            ...
            dir_vec = gp_Dir(math.sin(rad), 0.0, math.cos(rad))
    ...
    display.DisplayShape(shape, color="LIGHTGREEN", update=True)
