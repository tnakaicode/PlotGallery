...
table = translate_shp(BRepPrimAPI_MakeBox(
    100, 100, 10).Shape(), gp_Vec(-50, -50, -10))
glass_out = BRepPrimAPI_MakeCone(7, 9, 25).Shape()
glass_in = translate_shp(BRepPrimAPI_MakeCone(
    7, 9, 25).Shape(), gp_Vec(0.0, 0.0, 0.2))
...
dir_light = V3d_DirectionalLight(
    gp_Dir(0, 0, 1), Quantity_Color(Quantity_NOC_WHITE))
...
glass_pbr_aspect.SetBSDF(
    Graphic3d_BSDF().CreateGlass(
        weights, absorption_color, absorption_coef, refraction_index
    )
)

display.EnableAntiAliasing()
display.DisplayShape(bottle, material=alu_pbr_aspect)
display.DisplayShape(table, material=Graphic3d_NOM_PLASTIC,
                     color=Quantity_NOC_CORAL2)
display.DisplayShape(
    translated_glass, material=glass_pbr_aspect, transparency=0.8, update=True
)
