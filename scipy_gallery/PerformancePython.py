...
#     
# 
# The code itself looks very straightforward (which is what makes inline so
# cool). The inline function call arguments are all self explanatory. The line
# with '#line 120 ...' is only used, for debugging and doesn't affect the speed
# in anyway. Again the first time you run this function it takes a long while to
# do something behind the scenes and the next time it blazes away. This time
# notice that we have far more flexibility inside our loop and can easily
# compute an error term without a need for temporary arrays. Timing this version
# results in a time for a 500x500 array of a mere 0.04 seconds per iteration!
# This corresponds to a whopping 375 fold speed increase over the plain old for
# loop. And remember we haven't sacrificed any of Python's incredible
# flexibility! This loop contains code that looks very nice but if we want to we
# can speed things up further by writing a little dirty code. We won't get into
# that here but it suffices to say that its possible to get a further factor of
# two speed up by using a different approach. The code for this basically does
# pointer arithmetic on the NumPy array data instead of using blitz++ arrays.
# This code was contributed by Eric Jones. The source code accompanying this
# article contains this code.
# 
# Next, we look at how it is possible to easily implement the loop inside
# Fortran and call it from Python by using f2py.
# 
# ## Using f2py
# 
# f2py is an amazing utility that lets you easily call Fortran functions from
# Python. First we will write a small Fortran77 subroutine to do our
...
#           end
# 
# The lines starting with cf2py are special f2py directives and are documented
# in f2py. The rest of the code is straightforward for those who know some
# Fortran. We trivially create a Python module for this using the following
# command.
# 
#     
#     
...
#     
# 
# Thats it! Hopefully someday scipy.weave will let us do this inline and not
# require us to write a separate Fortran file. The Fortran code and f2py example
# were contributed by Pearu Peterson, the author of f2py. Anyway, using this
# module it takes about 0.029 seconds for a 500x500 grid per iteration! This is
# about a 500 fold speed increase over the original code.
# 
# f2py can also work with more modern Fortran versions. This is useful because
# Fortran90 has special array features that allow a more compact, nicer and
# faster code. Here is the same subroutine in Fortran90:
# 
#     
#     
...
#     end subroutine
# 
# Remark that the operations are performed in a single line, very similar to
# Numpy. The compilation step is exactly the same. In a 1000x1000 grid, this
# code is 2.2 times faster than the fortran77 loops.
# 
# The source tarball ( [perfpy_2.tgz](PerformancePython86aa.html?action=AttachFi
# le&do=view&target=perfpy_2.tgz "" ) ) also contains a Fortran95 subroutine
# using the forall construct, which has a very similar performance.
# 
# ## Using Pyrex/Cython
# 
# We also implemented the timeStep function in Pyrex using the code from the
# fast inline version. The Pyrex sources are a little longer than the weave,
# blitz or Fortran code since we have to expose the NumPy array structure. The
# basic function looks like this.
# 
# Travis Oliphant wrote a [Cython version](http://technicaldiscovery.blogspot.com/2011/06/speeding-up-python-numpy-cython-and.html) of the example.
#     
#     
