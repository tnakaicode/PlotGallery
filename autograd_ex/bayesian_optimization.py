...
def bayesian_optimize(func, domain_min, domain_max, num_iters=20, callback=None):
    ...
    def optimize_gp_params(init_params, X, y):
        def log_hyperprior(params): return np.sum(
            norm.logpdf(params, 0., 100.))

        def objective(params): return -log_marginal_likelihood(params,
                                                               X, y) - log_hyperprior(params)
    ...
    y = np.zeros((0))
    X = np.concatenate(
        (X, np.reshape((domain_max - domain_min) / 2.0, (D, 1))))
    ...
    for i in range(num_iters):
        ...
        def acquisition_function(xstar):
            # To work around a bug in scipy.minimize
            xstar = np.atleast_2d(xstar)
            mean, std = predict_func(xstar)
            return expected_new_max(mean, std, defaultmax(y))
        next_point = choose_next_point(
            domain_min, domain_max, acquisition_function)
...
if __name__ == '__main__':
    ...
    def callback(X, y, predict_func, acquisition_function, next_point, new_value):
        ...
        plot_xs = np.reshape(np.linspace(
            domain_min, domain_max, 300), (300, 1))
    ...
    best_x, best_y = bayesian_optimize(
        example_function, domain_min, domain_max, callback=callback)
