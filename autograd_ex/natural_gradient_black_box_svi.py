...
if __name__ == '__main__':
    ...
    def natural_gradient(lam, i): return (
        1. / fisher_diag(lam)) * gradient(lam, i)
    ...
    for step_size in step_sizes:
        ...
        def optfun(n, init, cb): return adam(gradient, init, step_size=step_size,
                                             num_iters=n, callback=cb)
        standard_lls = optimize_and_lls(optfun)

        # optimize with natural gradient + sgd, no momentum
        def optnat(n, init, cb): return sgd(natural_gradient, init, step_size=step_size,
