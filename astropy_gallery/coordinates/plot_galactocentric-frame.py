...
vy, vz = -(galcen_distance * pm_gal_sgrA).to(u.km /
                                             u.s, u.dimensionless_angles())
...
axes[1].set_xlabel('$v_x$ [{0}]'.format(
    (u.km / u.s).to_string("latex_inline")))
axes[1].set_ylabel('$v_y$ [{0}]'.format(
    (u.km / u.s).to_string("latex_inline")))
...
ax.set_xlabel('$l$ [deg]')
ax.set_ylabel(r'$\mu_l \, \cos b$ [{0}]'.format(
